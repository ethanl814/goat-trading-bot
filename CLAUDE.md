...
**Signals are organized by desk** (multistrat-fund style): `bot/framework/signals/<desk>/` where desk ∈ `equities/`, `prediction_markets/`, `crypto/`, `commodities/`. Put your signal in the matching desk folder. Discovery is recursive, so it's found automatically. **Every signal module starts with a "STRATEGY CARD" comment block** (name / desk / status / thesis / signal / entry-exit / risk / costs) — read it to know what a strategy does and where its risk is enforced; add one when you write a signal.

**Minimal example** — drop in `bot/framework/signals/<desk>/my_signal.py`:
```python
...
**First worked strategy — in-game run-reversal fade** (`signals/prediction_markets/sports_fade.py`, full writeup `docs/strategies/sports-run-fade.md`): a per-market state-machine signal (FLAT→LONG with reversion / stop / time-stop exits; **per-name risk lives in the signal**, book-level risk in `RiskMonitor`), paired with `EventFadeAllocator` (discrete fixed-count enter/hold/exit, optional conviction scaling — size ∝ overreaction). **Backtested profitable in the big/rare-overreaction regime** (taker +3% with conviction + fee-peak avoidance; maker +8.7% assuming limit fills). Tooling: `python -m scripts.backtest_sports_fade --mode taker|maker` and `python -m scripts.sweep_sports_fade` (grid search). **Realistic cost model:** `InstrumentSpec.fee_kind="kalshi"` (`ceil(0.07·C·p·(1−p))`, worst near p=0.5) + `slippage_price` (spread per fill), set by `build_spec`; maker mode sets fee/spread to 0. Lesson: the raw edge is small, so **only big/rare overreactions clear the taker spread** — bigger `entry_drop`, conviction sizing, and `avoid_lo/avoid_hi` (dodge the p≈0.5 fee peak) are what make it work. Deployable via `python -m bot.run live --venue kalshi` (start `MODE=PAPER`); `LiveKalshiBroker` is functional but fills aren't fully reconciled (market-only, YES-side). When adding a discrete event-trade strategy, use `allocator="event_fade"` and let the signal own entry/exit (the allocator just sizes).
